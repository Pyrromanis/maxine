...
import static com.oracle.max.vm.tests.vm.MaxineTester.Logs.*;
...
import com.oracle.max.vm.tests.vm.ExternalCommand.*;
import com.oracle.max.vm.tests.vm.ExternalCommand.Result;
import com.oracle.max.vm.tests.vm.output.*;
...
public class MaxineTester {
    ...
    private static final Option<Integer> javaRunTimeOutScale = options.newIntegerOption("timeout-scale", 100,
    ...
    private static final Option<Integer> junitTestTimeOutOption = options.newIntegerOption("junit-test-timeout", 125,
     ...
     */
    private static boolean addTestResult(String testName, String failure, MaxineTesterConfiguration.ExpectedResult expectedResult) {
        final boolean passed = failure == null;
        if (!expectedResult.matchesActualResult(passed)) {
            if (expectedResult == MaxineTesterConfiguration.ExpectedResult.FAIL) {
                unexpectedPasses.put(testName, failure);
            } else {
                assert expectedResult == MaxineTesterConfiguration.ExpectedResult.PASS;
    ...
    public static class JUnitTestRunner {
	    ...
	    //JUnitCore.runClasses(ParallelComputer.methods(),testClass	);
    ...
    private static boolean printMaxvmResult(String testName, String config, ExternalCommand.Result baseResult, ExternalCommand.Result maxResult, OutputComparison comparison) {
        ...
        boolean passed;
        final MaxineTesterConfiguration.ExpectedResult expectedResult = MaxineTesterConfiguration.expectedResult(testName, config);
        if (error != null) {
            // the test failed.
            ...
            } else if (expectedResult == MaxineTesterConfiguration.ExpectedResult.PASS) {
                out().print(left16(config + ": OK"));
            } else if (expectedResult == MaxineTesterConfiguration.ExpectedResult.NONDETERMINISTIC) {
    ...
    private static String getMaxvmErrorString(MaxineTesterConfiguration.ExpectedResult expectedResult) {
        if (expectedResult == MaxineTesterConfiguration.ExpectedResult.FAIL) {
            return "(normal)";
        } else if (expectedResult == MaxineTesterConfiguration.ExpectedResult.NONDETERMINISTIC) {
            return "(noluck)";
    ...
    public static class JUnitHarness implements Harness {
		...
		out().println("NO of Threads: "+threadCount);
         ...
         * {@linkplain MaxineTester#addTestResult(String, String, MaxineTesterConfiguration.ExpectedResult) updated} appropriately.
         *
        ...
        void parseTestResults(File resultsFile, boolean passed, Set<String> results, String jUnitTest) {
            try {
                ...
                for (String testName : lines) {
                    MaxineTesterConfiguration.ExpectedResult expectedResult = MaxineTesterConfiguration.expectedResult(jUnitTest, null);
                    if (!passed) {
